...
class modAlphaCipher {
    private:
        ...
        std::wstring getValidKey(const std::wstring & s);  // получение ключа
        std::wstring getValidOpenText(const std::wstring & s); // "чистка" открытого текста
        std::wstring getValidCipherText(const std::wstring & s); // "чистка" зашифр. текста
    public:
