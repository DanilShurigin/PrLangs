...
inline std::wstring modAlphaCipher::getValidKey(const std::wstring & s) {
    if (s.empty())
        throw cipher_error(L"Empty key");
    
    std::wstring tmp(s);

    for (auto & c:tmp) {
        if (!iswalpha(c))
            throw cipher_error(std::wstring(L"Invalid key ")+s);
        if (islower(c))
            c = towupper(c);
    }
...
std::wstring modAlphaCipher::getValidOpenText(const std::wstring & s) {
    ...
    if (tmp.empty())
        throw cipher_error(L"Empty open text");
...
std::wstring modAlphaCipher::getValidCipherText(const std::wstring & s) {
    if (s.empty())
        throw cipher_error(L"Empty cipher text");
    
    for (auto c:s) {
        if (!iswupper(c))
            throw cipher_error(std::wstring(L"Invalid cipher text ")+s);
    }
